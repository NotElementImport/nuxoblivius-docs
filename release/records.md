...
```
, принимающей 1-м аргументом путь к api; 2-м аргументом - значение для инициализации (по умолчанию - null).
...
* `lastPage` - номер последней страницы (определяется из meta-данных ответов через параметр `pageCount` в настройках [template](/release/template.html))
...
```
Подытоживая - `borrowAtAnother` принимает в аргументе 3 функции:
...
Функция, позволяющая `Record`-у при попытке очередного запроса к api (если их было несколько, например, взятие данных разных пользователей по id) взять данные из имеющегося кэша "старых" `Responsе`-ов по какому-либо условию.
