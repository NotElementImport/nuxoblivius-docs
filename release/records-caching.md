# Records Caching <Badge type="info" text="^ 1.1.0" style='margin-top: 7px;'/>  
...
Articles.article.cached({id: '*'}) // последняя запись при каком-либо выставленном id (любом)
```
...
console.log(Articles.articles.cached({page: '*'})) // лог, аналогичный предыдущему -  response статей при любом page (в т.ч. неуказанном)
...
console.log(Articles.articles.cached({id: '*'})) // все 5 вышеперечисленных логов вернут последний response при указанном любом id
